...
#define CONTAINS_Vec3(min, max, p) (min.x() <= p.x() && min.y() <= p.y() && min.z() <= p.z()) && (max.x() >= p.x() && max.y() >= p.y() && max.z() >= p.z())
...
template<typename T>
	struct AABB
	{
		T min = T(FLT_MAX, FLT_MAX, FLT_MAX);
		T max = T(-FLT_MAX, -FLT_MAX, -FLT_MAX);
		T center;
		...
		AABB(const AABB& other)
		{
			...
			diagonalLength = (min - max).length();
		}

		AABB(const T& a, const T& b, const T& c)
		{
			MIN_Vec3(min, a);
			MIN_Vec3(min, b);
			MIN_Vec3(min, c);
		...
		void Insert(const T& a, const T& b, const T& c)
		{
			MIN_Vec3(min, a);
			MIN_Vec3(min, b);
			MIN_Vec3(min, c);
		...
		{
			MIN_Vec3(min, other.min);
			MAX_Vec3(max, other.max);
		...
		{
			return CONTAINS_Vec3(min, max, p);
		}
