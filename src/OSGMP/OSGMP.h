...
//#define V3toVec3(v) Vec3(v.x.Float(), v.y.Float(), v.z.Float())
#define Vec3toV3(v) V3(v.x(), v.y(), v.z())
...
inline bool IsPointOnTriangle(const V3& p, const V3& v0, const V3& v1, const V3& v2)
{
	auto u = v1 - v0;
	auto v = v2 - v0;
	auto n = u ^ v;
...
public:
	...
	bool CheckIntersection(const string& nameA, const string& nameB, map<tuple<Mesh<T>*, Face<T>*>, set<pair<V3, V3>>>& result)
	{
		...
		VoxelSpace<Face<T>> vsA(1);
		VoxelSpace<Face<T>> vsB(1);
	...
	bool CheckIntersections(const string& baseName, const vector<string>& names, map<tuple<Mesh<T>*, Face<T>*>, set<pair<V3, V3>>>& result)
	{
	...
	void SplitFaces(const map<tuple<Mesh<T>*, Face<T>*>, set<pair<V3, V3>>>& input)
	{
		for (auto& kvp : input)
		{
